#!/usr/bin/env python3
"""M1 O2-oxidation Closure Analysis.

Read-only analysis of the O2 oxidation fixture JSON. Verifies mass, element
(C/O/H) closure, stoichiometric ratios (O2_per_C2, CO_per_C2), and tanh
damping ordering.

Reaction (O2 channel): C2(soot) + O2 -> 2CO
  O2 consumed: 1 per C2
  CO produced: 2 per C2
  Mass: soot loses 2C, gas gains 2CO - O2 = 2*28.010 - 31.999 = 24.021
...
def analyze_fixture(name, fx):
    ...
    if R_oxid_C2 > 0:
        ...
        # O: O2 provides 2O, CO takes 1O each
        O_in = -o2_rate * 1000 / MW["O2"] * 2.0
    ...
    # Reconstruct gas mass change from element moles x standard atomic weights.
    # For O2-ox: soot loses 2C per C2 -> gas gains 2C as CO. O and H balanced.
    AW_C = 12.011e-3  # kg/mol
    if R_oxid_C2 > 0:
        soot_mass_loss_elem = 2.0 * R_oxid_C2 * AW_C  # 2C atoms per C2
        gas_mass_gain_elem = soot_mass_loss_elem  # phase change, same composition
        mass_imb_elem_rel = (gas_mass_gain_elem - abs(soot_rate)) / abs(soot_rate) if abs(soot_rate) > 0 else 0.0
    else:
    ...
    return {
        ...
        "fO2": fO2, "fOH": fOH,
        "C_imb": C_imb, "O_imb": O_imb,
        "mass_imb_elem_rel": mass_imb_elem_rel,
    }


def main():
    parser = argparse.ArgumentParser(description="M1 O2-oxidation Closure Analysis")
    ...
    expected = ["A_all_off_control", "B_ox_above_threshold",
                "C_ox_below_threshold", "D_ox_near_threshold"]
    for n in expected:
    ...
    print("=" * 74)
    print("M1 O2-OXIDATION CLOSURE ANALYSIS")
    print(f"Source: {path}")
    print("=" * 74)
    ...
    A_ok = (abs(rA["soot_rate"]) < 1e-30 and
            all(abs(v) < 1e-30 for v in [rA["o2_rate"], rA["co_rate"], rA["oh_rate"], rA["h2_rate"]]))
    print(f"  A (all-off): {'PASS' if A_ok else 'FAIL'}")
    if not A_ok: all_closed = False

    rB = results["B_ox_above_threshold"]
    B_ok = (abs(rB["mass_imb_elem_rel"]) < CLOSURE_TOL and
            abs(rB["fO2"] - 1.0) < 0.01 and
            abs(rB["O2_per_C2"] - 1.0) < 0.01 and
            abs(rB["CO_per_C2"] - 2.0) < 0.01 and
            abs(rB["C_imb"]) < CLOSURE_TOL and
            abs(rB["O_imb"]) < 1e-4)
    print(f"  B (above threshold): {'PASS' if B_ok else 'FAIL'}")
    print(f"    mass_imb_elem = {sci(rB['mass_imb_elem_rel'])}, mass_imb_raw = {sci(rB['mass_imb_rel'])}")
    print(f"    fO2 = {sci(rB['fO2'])}, O2_per_C2 = {sci(rB['O2_per_C2'])}, CO_per_C2 = {sci(rB['CO_per_C2'])}")
    if not B_ok: all_closed = False

    rC = results["C_ox_below_threshold"]
    ...
    rD = results["D_ox_near_threshold"]
    ...
    if all_closed:
        ...
        print("  O2 channel: C2 + O2 -> 2CO verified (O2_per_C2=1, CO_per_C2=2)")
        print("  Mass, C, O balances close to < 1e-8 relative.")
